...
class CelebAMaskHQ(Dataset):
    ...
    def _load_target(self, img_path):
        ...
        target = torch.zeros((NUM_LABELS, 512, 512))
        for i, label in enumerate(LABEL_LIST, start=1):
            ...
            if os.path.exists(label_path):
                label = to_tensor(pil_loader(label_path))
                target[i, :, :] = label[0, :, :]

        target[0, :, :] = 1.0 - target[1:, :, :].max(dim=0)[0]  # background
        target = target.argmax(dim=0).int()
        return to_pil_image(target)
